...
namespace planet_generator
{
	...
	public:
		input() = delete;
		input(HWND hWnd, const std::vector<input_device_type> &devices);
		~input();

		void process_messages();

		bool test_keypress(key_code key) const;
		int16_t get_axis_relative(axis axis_) const;
		int16_t get_axis_absolute(axis axis_) const;

	private:
		void register_device(const std::vector<input_device_type> &devices);
		void process_raw_input(uint32_t count);
		void update_keys_pressed(const RAWKEYBOARD &data);
		key_code determine_key_code(uint16_t vKey, uint16_t sCode, uint16_t flags) const;
		void update_axis_data(const RAWMOUSE &data);
		void update_buttons_pressed(const RAWMOUSE &data);
		key_code determine_key_code(uint16_t btnFlags) const;
	...
	private:
		...
		std::array<bool, 256U> keys_pressed;
		std::array<int16_t, 4> axis_relative_positions;
		std::array<int16_t, 4> axis_absolute_positions;
	};

	enum class input::key_code : uint8_t
	{
		...
		OemQuestion = 0xbf, Oem2 = 0xbf, // Used for miscellaneous characters; it can vary by keyboard. For the US standard keyboard, the '/?' key
		OemTilde = 0xc0, Oem3 = 0xc0, // Used for miscellaneous characters; it can vary by keyboard. For the US standard keyboard, the '`~' key
		// 0xC1 - D7 are reserved
		// 0xD8 - DA are unassigned
		OemOpenBrackets = 0xdb, Oem4 = 0xdb, // Used for miscellaneous characters; it can vary by keyboard. For the US standard keyboard, the '[{' key
		OemPipe = 0xdc, Oem5 = 0xdc, // Used for miscellaneous characters; it can vary by keyboard. For the US standard keyboard, the '\|' key
		OemCloseBrackets = 0xdd, Oem6 = 0xdd, // Used for miscellaneous characters; it can vary by keyboard. For the US standard keyboard, the ']}' key
		OemQuotes = 0xde, Oem7 = 0xde, // Used for miscellaneous characters; it can vary by keyboard. For the US standard keyboard, the 'single-quote/double-quote' key
		Oem8 = 0xdf, // Used for miscellaneous characters; it can vary by keyboard.
		// 0xE0 is reserved
		// 0xE1 is OEM specific
		OemBackslash = 0xe2, Oem102 = 0xe2, // Either the angle bracket key or the backslash key on the RT 102-key keyboard
